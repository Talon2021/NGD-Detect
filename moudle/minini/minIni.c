...
static void writesection(TCHAR *LocalBuffer, const TCHAR *Section, INI_FILETYPE *fp, int add_newline)
{
  if (Section != NULL && _tcslen(Section) > 0) {
        TCHAR *p;
        if (add_newline && fp != NULL) {
            (void)ini_write(INI_LINETERM, fp);  // 添加一个换行符
        }
        LocalBuffer[0] = '[';
  ...
  if (!ini_openread(Filename, &rfp)) {
    ...
    if (Key != NULL && Value != NULL) {
      if (!ini_openwrite(Filename, &wfp))
        return 0;
      writesection(LocalBuffer, Section, &wfp, 0);
